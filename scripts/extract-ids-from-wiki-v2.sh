...
echo "export const RUNE_IDS = [" > /tmp/rune_ids.txt
...
while IFS= read -r name; do
    ...
    if [ -n "$id" ]; then
        ...
        echo "  $id, // $short_name" >> /tmp/rune_ids.txt
        echo "  Found: $name -> $id"
    else
        echo "  MISSING: $name"
    fi
...
echo "] as const;" >> /tmp/rune_ids.txt

echo ""
echo "Extracting relic IDs from wiki pages..."
echo "export const RELIC_IDS = [" > /tmp/relic_ids.txt
...
while IFS= read -r name; do
    ...
    if [ -n "$id" ]; then
        ...
        echo "  $id, // $short_name" >> /tmp/relic_ids.txt
        echo "  Found: $name -> $id"
    else
        echo "  MISSING: $name"
    fi
...
echo "] as const;" >> /tmp/relic_ids.txt

echo ""
echo "// Generated IDs from wiki pages"
cat /tmp/rune_ids.txt
echo ""
cat /tmp/relic_ids.txt
